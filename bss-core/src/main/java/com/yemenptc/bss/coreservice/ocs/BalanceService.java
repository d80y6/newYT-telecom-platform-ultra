...
import com.yemenptc.bss.coreservice.exception.ConcurrentChargingException;
...
public class BalanceService {
    ...
    @Transactional
    public BalanceResult deductBalanceSafely(String accountId, BigDecimal amount) {
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            return BalanceResult.builder()
                    .success(false)
                    .errorCode("INVALID_AMOUNT")
        ...
        try {
            lockAcquired = lock.tryLock(0, LOCK_LEASE_TIME_SECONDS, TimeUnit.SECONDS);

            if (!lockAcquired) {
                log.warn("Failed to acquire lock for account {} - concurrent charging detected", accountId);
                throw new ConcurrentChargingException("Unable to acquire lock for account: " + accountId + " - concurrent charging in progress");
            }
            ...
            String transactionId = UUID.randomUUID().toString();
            log.info("Safely deducted {} from account {}, transaction {}", amount, accountId, transactionId);
            ...
            log.error("Lock acquisition interrupted for account {}", accountId);
            throw new ConcurrentChargingException("Lock acquisition interrupted for account: " + accountId, e);
        } finally {
    ...
    public static class BalanceResult {
        ...
        public String message() { return message != null ? message : errorMessage; }
