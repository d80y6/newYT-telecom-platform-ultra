...
@RestController
@RequestMapping("/tmf-api/customerManagement/v5/account")
...
public class CustomerAccountController {
    ...
    public ResponseEntity<TmfResponse<Account>> create(@RequestBody Account request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(TmfResponse.success(account, "Account"));
    }

    @GetMapping("/{id}")
    public ResponseEntity<TmfResponse<Account>> getById(@PathVariable UUID id) {
        Account account = accountService.getAccount(id);
        return ResponseEntity.ok(TmfResponse.success(account, "Account"));
    }
    ...
    public ResponseEntity<TmfResponse<Account>> list(
        ...
        return ResponseEntity.ok(TmfResponse.list(
                ...
                page * size, size, "Account"));
    }

    @PutMapping("/{id}")
    public ResponseEntity<TmfResponse<Account>> update(
            @PathVariable UUID id, @RequestBody Account request) {
